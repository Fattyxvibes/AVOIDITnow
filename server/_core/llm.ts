...
const assertApiKey = () => {
  if (!ENV.aiApiKey || !resolveApiUrl()) {
    throw new Error("An OpenAI-compatible AI service is not configured.");
  }
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const selectedModel = model ?? ENV.aiModel;
  if (selectedModel) {
...
/**
 * Starts an OpenAI-compatible server-sent event response without exposing the
 * built-in Forge credential to the browser. The caller owns reading/cancelling
 * the returned stream.
 */
export async function invokeLLMStream(
  ...
  const selectedModel = params.model ?? ENV.aiModel;
