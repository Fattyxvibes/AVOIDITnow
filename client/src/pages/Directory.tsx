...
export default function Directory() {
  ...
  const suggestionInput = useMemo(() => ({ query: query.trim() }), [query]);
  const suggestions = trpc.platform.productSuggestions.useQuery(suggestionInput, {
    enabled: showSuggestions && query.trim().length >= 1,
    staleTime: 30_000,
    retry: false,
  });
  ...
  const chooseSuggestion = (suggestion: { name: string }) => {
    setQuery(suggestion.name);
    setShowSuggestions(false);
    requestPolicyAgreement(() => setSubmittedQuery(suggestion.name));
  };
      ...
      <ViewportReveal delay={80}><form onSubmit={submit} className="rounded-2xl border border-black/10 bg-white p-3 shadow-[0_10px_30px_rgba(20,33,23,.06)]">
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative min-w-0 flex-1">
            <label className="flex min-w-0 items-center gap-3 rounded-xl bg-[#f3f6f1] px-4">
              ...
              <Input
                ...
                aria-expanded={showSuggestions && Boolean(suggestions.data?.length)}
                aria-controls="product-suggestions"
              />
            </label>
            {showSuggestions && suggestions.data?.length ? (
              <div id="product-suggestions" role="listbox" aria-label="Product suggestions" className="absolute inset-x-0 top-[calc(100%+0.5rem)] z-20 overflow-hidden rounded-xl border border-[#137547]/15 bg-white p-2 shadow-[0_18px_40px_rgba(20,33,23,.14)]">
                <p className="px-3 pb-1 pt-2 text-[11px] font-extrabold uppercase tracking-[.14em] text-[#667169]">Do you mean?</p>
                {suggestions.data.map(suggestion => (
                  <button
                    key={`${suggestion.name}-${suggestion.subproduct}`}
                    type="button"
                    role="option"
                    aria-label={`Check ${suggestion.name}`}
                    onMouseDown={event => event.preventDefault()}
                    onClick={() => chooseSuggestion(suggestion)}
                    className="flex min-h-12 w-full items-start justify-between gap-3 rounded-lg px-3 py-2 text-left transition hover:bg-[#f1f7f1] focus-visible:bg-[#f1f7f1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#137547]"
                  >
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-bold text-[#172119]">{suggestion.name}</span>
                      <span className="mt-0.5 block truncate text-xs text-[#667169]">{suggestion.subproduct}</span>
                    </span>
                    <span className="shrink-0 pt-0.5 text-xs font-bold text-[#137547]">Check</span>
                  </button>
                ))}
              </div>
            ) : null}
